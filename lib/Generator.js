...
export default class Generator {
    ...
    text(text) {
        if (text) {
            ...
            if (Array.isArray(text)) {
                ...
                bytes = cptable.utils.encode(this._page_table, text);
            }
    ...
    textColumns(values) {
        ...
        if (!done) {
            let currentAvailableChar = this._max_characters_per_line;
            ...
            values.forEach((item) => {
                if (!item.percentage) {
                    ...
                    let toAlloc = parseInt(this._max_characters_per_line / 100 * item.percentage);
            ...
            if (toBeAutomaticallyAllocated.length) {
                let spacePerItem = parseInt(currentAvailableChar / toBeAutomaticallyAllocated.length);
                ...
                item.__text_array.forEach((singleText) => {
                    function recursiveCut(text, max_chars, processed) {
                        if (text.length > max_chars) {
                            ...
                            recursiveCut(text.substring(max_chars, text.length), max_chars, processed);
                        } else {
                            processed.push(text);
                        }
                    }
                ...
                values.forEach((item) => {
                    ...
                    for (let i = 0; i < item.__text_array.length; i++) {
                        ...
                        if (item.__text_array[i].length <= remainingCharsToBeAllocated) {
                            bytesToWrite = bytesToWrite.concat(cptable.utils.encode(this._page_table, item.__text_array[i]));
    ...
    get maxCharactersPerLine() {
        return this._max_characters_per_line;
    }

    //Static variables
    static get MODALITY_CLASS() { return "class"; }
    static get MODALITY_ARRAY() { return "array"; }
    static get MODALITY_DIRECT() { return "direct"; }

    static get LEFT() { return "left"; }
    static get CENTER() { return "center"; }
    static get RIGHT() { return "right"; }

    static get CUT_PAPER_PARTIAL() { return "partial"; }
    static get CUT_PAPER_FULL() { return "full"; }

    static get FONT_NORMAL() { return 0b00000000 };
    static get FONT_DOUBLE() { return 0b00010001 };
    static get FONT_TRIPLE() { return 0b00100010 };
}